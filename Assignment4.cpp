...
void callback_start(int id) {
	...
	//eye point eyeX = 2,eyeY =2 ,eyeZ = 2 
	for(int x = 0;x<500;x++){
		if(x %25 ==0)
			std::cout<<x<<"	";
		...
		for(int y = 0;y<screenlength;y++){
			//the eye point p
			// calculate the ray and direction d
	...
	std::cout<<endl<<"render complete"<<endl;
}
...
/*
	Draw the primitive p.
*/
void drawPrimitive(const ScenePrimitive* p, bool applyMat){
	if (applyMat){
...
/*
	Apply all of the transformations in the given vector.
*/
void applyTransformation(const std::vector<SceneTransformation*> transforms){
	...
	for (int i = 0; i < numTrans; i++){
		...
		case TRANSFORMATION_TRANSLATE:
			values = transforms[i]->translate;
			glTranslatef(values[0], values[1], values[2]);
			break;
		case TRANSFORMATION_ROTATE:
			values = transforms[i]->rotate;
			rot = rot_mat(values, transforms[i]->angle);
			glMultMatrixd(rot.unpack());
			break;
		case TRANSFORMATION_SCALE:
			values = transforms[i]->scale;
			glScalef(values[0], values[1], values[2]);
			break;
		case TRANSFORMATION_MATRIX:
			glMultMatrixd(transforms[i]->matrix.unpack());
			break;
		}
	}
}
...
/*
	Recursively draws all of the objects in the scene tree, starting with the node
	that is passed as a parameter.
*/
void draw(const SceneNode* root, bool mat = true){
	...
	applyTransformation(root->transformations);

	int numPrim = root->primitives.size();
	for (int i = 0; i < numPrim; i++){
		drawPrimitive(root->primitives[i], mat);
	}

    int numChild = root->children.size();
    for (int i = 0; i < numChild; i++){
        draw(root->children[i], mat);
    }
	glPopMatrix();
}


void draw_isect(const SceneNode* root, bool mat = true){
...
void myGlutDisplay(void)
{
	...
	if (wireframe) {
        ...
        //TODO: draw wireframe. No material
		draw(root, false);
	}
...
int main(int argc, char* argv[])
{
	...
	new GLUI_Checkbox(glui, "Isect Only", &isectObj);
